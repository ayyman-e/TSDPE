...
def resample_signals(data, old_fs, new_fs):
    ...
    for channel in range(data.shape[1]):
        ...
        if old_fs != new_fs:
            ...
            print(
                "The current sampling rate matches the fitted sampling rate. No resampling required."
            )
            channel_results.append(channel_data)
...
def period_batching(data, no_periods):
    zero_crossings = [
        ...
        if data[idx - 1, 0] > float(0) >= data[idx, 0]
    ]
    return [
        data[zero_crossings[idx] : zero_crossings[idx + no_periods]]
        for idx in range(1, len(zero_crossings) - no_periods, no_periods)
    ]
